...
def main(rank, world_size, graph_list, seed=0):
    ...
    ckpt_freq = 1000
    log_freq = 1
    ckpt_dir = './cases/powerDrop_dist_2parts_test/saved_model'
    log_dir = './cases/powerDrop_dist_2parts_test'
    ...
    for epoch in range(epochs + 1):
        ...
        for _, (G_feat, G_target) in enumerate(train_loader):
            ...
            loss_1 = F.mse_loss(pred_1['loop'], G_target.nodes['loop'].data['feat'], reduction = 'sum')
...
def init_model_future(seed, graph_name, device):
    ...
    n_hid = 32
    n_input = {'loop':3, 'core':2, 'pump':1}
    model = NodeFuturePredictor(graph=graph_template[0], n_inp=n_input, n_hid=n_hid , n_layers=2, n_heads=1, time_window=10, norm=False,device = device)
    model = model.to(device)
    if device.type == 'cpu':
        model = DistributedDataParallel(model)
    else:
